...
- **Mermaid 図**: ```` ```mermaid ```` のコードブロックを図として描画（`mermaid`）。クリックで全画面に開き、**ピンチで拡大縮小 / 2 本指スクロールで移動**できます（Cmd + スクロールでも拡大縮小、マウスならドラッグで移動）。拡大するとベクタから描き直すので、どこまで寄せても輪郭は鮮明です。描画はすべて端末内で行うため、図の内容が外部へ送られることはありません。`mermaid` は本体が大きいので、**図を含むメモを開いたときだけ**読み込みます（図の無いメモでは起動も表示も従来どおり）。構文が不正な図はコードブロックのまま残し、理由を 1 行添えます
- **本文幅**: プレビューの右端をドラッグすると本文の幅を変えられます（ダブルクリックで既定の 860px に戻す）。幅はこの端末に保存され、次回起動時も維持されます
- **メモ間リンク**: 本文に `[[メモ名]]`（`.md` を除いたファイル名）と書くとプレビューでリンクになり、クリックでそのメモへ移動します。表示はリンク先の**タイトル**（本文の先頭行）に差し替わるので、`[[note-1784965179337]]` のような機械的なファイル名でも中身が分かります。`[[メモ名|表示]]` と書けば表示を固定でき、標準の `[表示](メモ名.md)` も同じように辿れます。リンク先が無い場合は薄く表示し、クリックすると知らせます
...
| `Ctrl/Cmd + F` | 見ている側で検索（エディタ内 / プレビュー内）。プレビューでは `Enter` / `Shift + Enter` で次・前へ |
